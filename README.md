...
- 大板块首屏分批渲染，可继续“加载更多”，并可展开查看每个信源的在线状态和当期条数；
- 提供 **Signal Console（高密度控制台）** 与 **Editorial Brief（编辑简报）** 两种可持久化切换的阅读模式；
- 支持紧凑/舒展密度切换、实时状态轨道、键盘快速导航与移动端侧栏。
...
- **控制台 / 简报**：右上角切换两种结构化信息视图，选择会保存在浏览器中；
- **密度**：在紧凑与舒展显示之间切换；
- **1–4**：切换 AI、中国、全球和实时导航；
- **/**：聚焦搜索框；
- **V**：快速切换控制台与简报模式；
- 新闻条目始终直接打开原始发布机构页面。
